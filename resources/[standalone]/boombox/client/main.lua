...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("boombox:joined")
end)
...
RegisterNUICallback("loadServer",function(data, cb)
	...
	local tempTable = {
		...
		TriggerServerEvent("boombox:loadSpeaker", tempTable[1])
end)


RegisterNetEvent("boombox:loadSpeakerClient")
AddEventHandler("boombox:loadSpeakerClient", function(speaker)
	cacheSpeaker[speaker.speaker] = speaker
end)


RegisterNetEvent("boombox:joined")
AddEventHandler("boombox:joined", function(speaker)
	cacheSpeaker = speaker
end)

RegisterNetEvent("boombox:removeClient")
AddEventHandler("boombox:removeClient", function(speaker)
...
RegisterNetEvent("boombox:switchVideoClient")
AddEventHandler("boombox:switchVideoClient", function(id, videoStatus, time)
	cacheSpeaker[id].switch = true
	cacheSpeaker[id].videoStatus = videoStatus
	cacheSpeaker[id].time = time - cacheSpeaker[id].time
end)

RegisterNetEvent("boombox:changeVolClient")
AddEventHandler("boombox:changeVolClient", function(id, vol)
                ...
                if v.id ~= nil then
					...
					if dist < 250.0 then
						...
						elseif playing == k and onSwitch then
							...
							TriggerServerEvent("boombox:switchVideo", k, videoStatus, newTime)
						...
						elseif playing == k and onVolChange then
							...
							TriggerServerEvent("boombox:changeVol", k, volchange)
			...
			for k, v in pairs(cacheSpeaker) do
				...
				if #(v.coords - GetEntityCoords(PlayerPedId())) < 2.0 then
					...
					if IsControlJustPressed(0, 47) then
						...
						TriggerServerEvent("boombox:removeSpeaker", currentId)
...
AddEventHandler("boombox:place", function()
	local plyrcoords, forward = GetEntityCoords(PlayerPedId()), GetEntityForwardVector(PlayerPedId())
	local spawncoords = (plyrcoords + forward * 1.0)
	tooClose = false
	if #cacheSpeaker > 0 then
		...
		if not tooClose then
			...
			TriggerServerEvent("boombox:placedSpeaker", spawncoords, speaker)
			SetEntityHeading(speaker, GetEntityHeading(PlayerPedId()))
		else
	--		ESX.ShowNotification(Config["translations"].tooClose, false, false)
			QBCore.Functions.Notify(Config["translations"].tooClose, 'error')
		end
	else
		speaker = CreateObject(1729911864, spawncoords, true, true, true)
		FreezeEntityPosition(speaker, true)
