...
function outOfPins() {
  if(Lockpickdoor == true)
  {
	  gameOver = true;
	  $('#divp').css('display','none');
	  $.post('http://LockpickUi/losedoor', JSON.stringify({}));
	  location.reload();
  }
  else
  {
	  gameOver = true;
	  $('#divp').css('display','none');
	  $.post('http://LockpickUi/lose', JSON.stringify({}));
	  location.reload();
  }
}
...
function unlock() {
  if(Lockpickdoor == true)
  {
	  gameOver = true;
	  $('#divp').css('display','none');
	  $.post('http://LockpickUi/windoor', JSON.stringify({}));
	  location.reload();
  }
  else
  {
	  gameOver = true;
	  $('#divp').css('display','none');
	  $.post('http://LockpickUi/win', JSON.stringify({}));
	  location.reload();
  }
}
...
$(document).ready(function(){
 ...
 window.addEventListener( 'message', function( event ) {
        ...
        if ( item.showPlayerMenu == true ) {
            ...
            $('#divp').css('display','block'); //Lockpickdoor
			if (item.typebekhoa == false )
			{
				pinDamage = 20
			}
			console.log(item.typedoor)
			if (item.typedoor == true )
			{
				Lockpickdoor = true // ok roi do
				console.log(Lockpickdoor)
			}
			else
			{
				Lockpickdoor = false // ok roi do
				console.log(Lockpickdoor)
			}
