...
CreateThread(function()
    while true do
        if isWithinObject and oElement.fObject ~= 0 then
            ...
            if (distanceDiff < 1.5 and not InUse) then
                if (oElement.fObjectIsBed == true) then
                    ...
                    if (Anim == 'sit') then
                        DrawText3Ds(objectCoords.x, objectCoords.y, objectCoords.z, "~b~[E]~s~ Ngồi")
                    ...
                    if IsControlJustPressed(0, Config.objects.ButtonToSitOnChair) then
                        TriggerServerEvent('ChairBedSystem:Server:Enter', oElement, oElement.fObjectCoords)
                        Framework.Functions.Notify("Đặt đít xuống ghế", primary, 1000)
                    end
                end     
            end
            ...
            if (InUse) then
                ...
                if IsControlJustPressed(0, Config.objects.ButtonToStandUp) or GetEntityHealth(PlayerPedId(source)) == 0 then
                    Framework.Functions.Notify("Nhấc đít ra khỏi ghế", primary, 1000) -- [text] = message, [type] = primary | error | success, [length] = time till fadeout.
                    InUse = false
                    ...
                    if GetDistanceBetweenCoords(x, y, z, plyCoords) < 1 then
...
CreateThread(function()
    ...
    while true do
        for _, element in pairs(Config.objects.locations) do
            ...
            if (distanceDiff < 2.0 and closestObject ~= 0) then
