        ...
        if isWithinObject and oElement.fObject ~= 0 then
            ...
            if (distanceDiff < 2.0 and not InUse) then
                if (oElement.fObjectIsBed == true) then
                    ...
                    if IsControlJustPressed(0, Config.objects.ButtonToLayOnBed) then
                        TriggerServerEvent('pd-chairsystem:Server:Enter', oElement, oElement.fObjectCoords)
                    end
                else    
            ...
            if (InUse) then
                ...
                if IsControlJustPressed(0, Config.objects.ButtonToStandUp) then
                    InUse = false
                    TriggerServerEvent('pd-chairsystem:Server:Leave', oElement.fObjectCoords)
...
RegisterNetEvent('pd-chairsystem:Client:Animation')
AddEventHandler('pd-chairsystem:Client:Animation', function(v, coords)
