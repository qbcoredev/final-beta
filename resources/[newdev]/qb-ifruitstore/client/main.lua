                ...
                if dist < 1.0 then
                    ...
                    if dist < 0.6 then
                        ...
                        if not Config.Locations["takeables"][spot].isBusy and not Config.Locations["takeables"][spot].isDone then
                            DrawText3Ds(Config.Locations["takeables"][spot].x, Config.Locations["takeables"][spot].y,Config.Locations["takeables"][spot].z, '~g~E~w~ Để lấy vật phẩm')
                                    ...
                                    if Config.Locations["thermite"].isDone then 
                                        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
                                            if hasItem then
                                                ...
                                                QBCore.Functions.Notify("Bạn đang thiếu khóa vạn năng cao cấp", "error")
                                            end
                                        end, "advancedlockpick")
                                    else
                                        QBCore.Functions.Notify("Bảo mật vẫn còn hoạt động..", "error")
                                    end
                                else
                                    QBCore.Functions.Notify("Cảnh sát không đủ", "error")
                                end
                            end
                        end
                    else
...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
            QBCore.Functions.Notify("Bạn làm rách găng tay của bạn..")
        end
...
function GrabItem(spot)
    ...
    QBCore.Functions.Progressbar("grab_ifruititem", "Disconnect Item", 10000, false, true, {
        ...
        TriggerServerEvent('qb-ifruitstore:server:PoliceAlertMessage', 'Có ai đó cố gắng đột nhập tại Apple Store', pos, true)
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
        TriggerServerEvent('qb-ifruitstore:server:setSpotState', "isBusy", false, spot)
...
AddEventHandler('thermite:UseThermite', function()
    ...
    if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z)) < 1.0 then
        if CurrentCops >= 0 then
            if cooldown then
                QBCore.Functions.Notify("Vui Lòng Chờ 15 Phút", "error")
            else
                local pos = GetEntityCoords(PlayerPedId())
                if math.random(1, 100) <= 80 and not IsWearingHandshoes() then
                    TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
...
AddEventHandler('qb-ifruitstore:client:PoliceAlertMessage', function(msg, coords, blip)
    if blip then
        ...
        TriggerEvent("chatMessage", "Cảnh Báo An Ninh", "error", msg)
        local transG = 100
        ...
        AddTextComponentString("911 - Tình huống đáng ngờ trong tại Apple Store")
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent('qb-ifruitstore:server:PoliceAlertMessage', 'Có ai đó cố gắng đột nhập tại Apple Store', coords, true)
end)

RegisterNUICallback('thermitesuccess', function()
    QBCore.Functions.Notify("Đã Phá Hủy Các Cầu Chì", "success")
    TriggerServerEvent("QBCore:Server:RemoveItem", "thermite", 1)
    local pos = GetEntityCoords(PlayerPedId())
    if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z)) < 1.0 then
...
AddEventHandler('qb-ifruitstore:client:robberyCall', function(streetLabel, coords)
    if PlayerJob.name == "police" then 
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("10-90: Xảy Ra Cướp Tại Apple Store ")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
