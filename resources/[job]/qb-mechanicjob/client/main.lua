...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Trung Tâm Sửa Xe")
            ...
            if PlayerJob.name == "mechanic" then
                ...
                if onDuty then
                    if StashDistance < 20 then
                        ...
                        if StashDistance < 1 then
                            DrawText3Ds(Config.Locations["stash"].x, Config.Locations["stash"].y, Config.Locations["stash"].z, "[E] Mở Kho Đồ")
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Dừng Ca Trực")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Nhận Ca Trực")
                        end
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("QBCore:ToggleDuty")
                        end
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "[E] Đặt Xe")
                                            if IsControlJustPressed(0, 38) then
                                                DoScreenFadeOut(150)
                                                Wait(150)
...
function VehicleOptions()
    ClearMenu()
    Menu.addButton("Lấy Xe ra", "UnattachVehicle", nil)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Sửa ("..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("trở lại", "VehicleOptions", nil)
    Menu.addButton("đóng Menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Notify('bạn không có đủ vật liệu trong kho', 'error')
        end
    end, "mechanicstash")
end
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        QBCore.Functions.Notify("Trạng thái không xác định", "error")
                    end
                else
                    QBCore.Functions.Notify("Không phải là một chiếc xe hợp lệ", "error")
                end
            else
                QBCore.Functions.Notify("Bạn không ở gần xe", "error")
            end
        else
...
RegisterNetEvent('toggledutymcn')
AddEventHandler('toggledutymcn', function()
    onDuty = not onDuty
    TriggerServerEvent("QBCore:ToggleDuty")
end)
