    ...
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                QBCore.Functions.Notify('NPC được chỉ định trên GPS của bạn', 'success')
                    ...
                    while not NpcData.NpcTaken do
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                DrawText3D(Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].x, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].y, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].z, '[E] Mời Khách Lên Xe')
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    QBCore.Functions.Notify('Đưa họ đến vị trí trên bản đồ')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                QBCore.Functions.Notify('Bạn đã thực hiện một nhiệm vụ')
            end
      --  else
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    local npccoords = GetEntityCoords(NpcData.Npc)
                    DrawText3D(Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].x, Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].y, Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].z, '[E] Thả Khách')
                    if IsControlJustPressed(0, 38) then
                        local veh = GetVehiclePedIsIn(ped, 0)
                        ...
                        QBCore.Functions.Notify('Khách Hàng Đã Xuống', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
            ...
            if isLoggedIn then
                ...
                if PlayerData.job.name == "taxi" then
                    ...
                    if vehDist < 30 then
                        ...
                        if vehDist < 1.5 then
                            if whitelistedVehicle() then
                                DrawText3D(Config.Location.x, Config.Location.y, Config.Location.z + 0.3, '[E] Đậu Xe')
                                ...
                                DrawText3D(Config.Location.x, Config.Location.y, Config.Location.z + 0.3, '[E] Xe Công Việc')
                                if IsControlJustReleased(0, 38) then
                                    TaxiGarage()
