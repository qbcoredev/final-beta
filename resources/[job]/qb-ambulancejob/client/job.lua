    ...
    QBCore.Functions.TriggerCallback('hospital:server:HasFirstAid', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                QBCore.Functions.Progressbar("hospital_revive", "Cấp Cứu Người chơi..", 5000, false, true, {
                ...
                QBCore.Functions.Notify("Không có người gần đó", "error")
            end
        else
            QBCore.Functions.Notify("Bạn cần có bộ sơ cứu", "error")
        end
    end, 'firstaid')
end)
...
AddEventHandler('hospital:client:TreatWounds', function()
    QBCore.Functions.TriggerCallback('hospital:server:HasBandage', function(hasItem)
        if hasItem then
            ...
            QBCore.Functions.Notify("Bạn cần cái băng gạc", "error")
        end
    end, 'bandage')
end)
