        ...
        if QBCore ~= nil then
            ...
            if #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 1.5 then
                if doctorCount >= Config.MinimalDoctors then
                    ...
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Nhập Viện")
                end
                if IsControlJustReleased(0, 38) then
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        QBCore.Functions.Progressbar("hospital_checkin", "Đăng kí nhập viện", 2000, false, true, {
                            ...
                            if bedId ~= nil then 
                                ...
                                QBCore.Functions.Notify("Giường dành cho bệnh nhân đã hết..", "error")
                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
            ...
            elseif #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 4.5 then
                if doctorCount >= Config.MinimalDoctors then
                    ...
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Đăng Kí")
                end
            end
...
AddEventHandler('hospital:client:Revive', function()
    ...
    QBCore.Functions.Notify("Bạn hoàn toàn khỏe mạnh !")
end)
...
function OutfitsLijst()
    QBCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
            QBCore.Functions.Notify("Bạn đã không lưu bất kỳ trang phục...", "error", 3500)
            closeMenuFull()
        else
