...
--[[ Citizen.CreateThread(function()
    while true do
        ...
        TriggerClientEvent("ambulance:SetCopCount", -1, curCops)
    end
end) ]]
...
AddEventHandler('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney("bank", Config.Bedcost)
	--TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
	TriggerEvent("qb-bossmenu:server:addAccountMoney", "ambulance", Config.Bedcost)
end)
...
AddEventHandler('hospital:server:RespawnAtHospital', function()
	...
	for k, v in pairs(Config.Locations["beds"]) do
		...
		TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
		TriggerEvent("qb-bossmenu:server:addAccountMoney", "ambulance", Config.BillCost * 0.10)
		return
	end
end)
...
--[[ RegisterServerEvent('ambulance:server:UpdateCurrentCops')
AddEventHandler('ambulance:server:UpdateCurrentCops', function()
    local amount = 0
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
    ...
    TriggerClientEvent("ambulance:SetCopCount", -1, amount)
end) ]]