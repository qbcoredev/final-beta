...
AddEventHandler('police:server:TakeOutImpound', function(plate)
    ...
    TriggerClientEvent('QBCore:Notify', src, "Phương tiện được đưa ra khỏi Khu Giam Giữ !")
end)
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Bạn đã đưa người đó vào tù  " .. time .. " phút")
        end
    end
end)
...
AddEventHandler('police:server:SearchPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Bạn Đang Bị Lục Soát..")
    end
end)
...
AddEventHandler('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Bạn Bị Thu Tiền Mặt.")
    end
end)
...
AddEventHandler('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Bạn Đã Bị Lấy $" .. money)
        TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "Bạn đã lấy $" .. money)
    end
end)
...
RegisterServerEvent('police:server:Depot')
AddEventHandler('police:server:Depot', function(plate, fullImpound, price, body, engine, fuel)
    local src = source
    ...
    if IsVehicleOwned(plate) then
        if price >= 2000 and price <= 5000 then
            if not fullImpound then
                ...
                TriggerClientEvent('QBCore:Notify', src, "Xe đưa vào kho $" .. price .. "!")
            else
                exports.oxmysql:execute(
                    'UPDATE player_vehicles SET state = ?, body = ?, engine = ?, fuel = ? WHERE plate = ?',
                    {2, body, engine, fuel, plate})
                TriggerClientEvent('QBCore:Notify', src, "Phương Tiện Bị Thu Giữ!")
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Không Thể Giam Xe Với Số Tiền Như Vậy Như Vậy!")
        end
    end
end)
...
AddEventHandler('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Bạn phải có một túi bằng chứng rỗng bên mình", "error")
    end
end)
...
AddEventHandler('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('QBCore:Notify', targetId, 'Vòng Tay Của bạn đã tháo ra.', 'error', 5000)
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if SearchedPlayer ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Bạn đã được cấp giấy phép",
                    "success", 5000)
                TriggerClientEvent('QBCore:Notify', source, "Bạn đã cấp giấy phép", "success", 5000)
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Loại giấy phép không hợp lệ", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "Bạn phải là một trung sĩ để cấp giấy phép!", "error")
    end
end)

QBCore.Commands.Add("revokelicense", "Thu hồi giấy phép từ ai đó", {{
    name = "id",
    help = "ID of a person"
}, {
    name = "license",
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if SearchedPlayer ~= nil then
                ...
                SearchedPlayer.Functions.SetMetaData("licences", licenseTable)
                TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Bạn đã bị thu hồi giấy phép",
                    "error", 5000)
                TriggerClientEvent('QBCore:Notify', source, "Bạn đã thu hồi giấy phép", "success", 5000)
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Loại giấy phép không hợp lệ", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "Bạn phải là một trung sĩ để thu hồi giấy phép!", "error")
    end
end)
...
QBCore.Commands.Add("cam", "Xem camera an ninh (Police Only)", {{
    name = "camid",
    help = "Camera ID"
}}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("depot", "Impound With Price (Police Only)", {{
    ...
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        TriggerClientEvent("police:client:depot", source, false, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', source, 'For Emergency Services Only', 'error')
    end
end)
