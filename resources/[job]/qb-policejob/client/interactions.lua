...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            QBCore.Functions.Notify("Thời gian phải cao hơn 0..", "error")
        end
    else
        QBCore.Functions.Notify("Không Có Ai Ở Gần!", "error")
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                ...
                QBCore.Functions.Notify("Không Thể Còng Tay Trên Xe", "error")
            end
        else
            QBCore.Functions.Notify("Không Có Ai Ở Gần!", "error")
        end
            ...
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    ...
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                        ...
                        QBCore.Functions.Notify("Bạn không thể còng ai đó trong một chiếc xe", "error")
                    end
                else
                    QBCore.Functions.Notify("Bạn không có còng tay", "error")
                end
            end, "handcuffs")
        else
            QBCore.Functions.Notify("Không Có Ai Ở Gần!", "error")
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            QBCore.Functions.Notify("Bạn Đã Bị Còng Tay!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            QBCore.Functions.Notify("Bạn Bị Còng Tay Nhưng Có Thể Di Chuyển")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(PlayerPedId())
        QBCore.Functions.Notify("Bạn Đã Được Mở Còng Tay!")
    end
end)
