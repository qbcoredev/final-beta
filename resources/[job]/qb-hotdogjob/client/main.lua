...
function UpdateBlip()
    Citizen.CreateThread(function()
        ...
        AddTextComponentSubstringPlayerName("Đại Lý Hotdog")
                ...
                if distance < 10 then
                    ...
                    if not IsWorking then
                        if distance < OffsetData.Distance then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Bắt đầu làm việc")
                            if IsControlJustPressed(0, 38) then
                                StartWorking()
                            end
                        elseif distance < 3 then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "Bắt đầu làm việc")
                        end
                    else
                        if distance < OffsetData.Distance then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Ngừng Làm Việc")
                            if IsControlJustPressed(0, 38) then
                                StopWorking()
                            end
                        elseif distance < 3 then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "Ngừng Làm Việc")
                        end
                    end
                end
            end
...
function StartWorking()
    QBCore.Functions.TriggerCallback('qb-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            ...
            UpdateUI()
            CheckLoop()
            QBCore.Functions.Notify('Bạn Đã Thanh Toán $250 Tiền Cọc Xe Hotdog!..', 'success')
        else
            QBCore.Functions.Notify('Bạn Không Có Đủ Tiền Để Cọc..', 'error')
        end
    end)
end
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if not IsPushing then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[G] Đẩy Xe Bán Hàng')
                            ...
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[G] Dừng Lại')
                            if IsControlJustPressed(0, 47) then
                                LetKraamLose()
                            end
                        end
                    elseif ObjectDistance < 3.0 then
                        DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, 'Đẩy Xe')
                    end
                end
            else
                break
        ...
        while true do
            ...
            if StandObject ~= nil then
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if SellingData.Enabled then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Để Làm HotDog [Trạng Thái: ~g~Đang Bán~w~]')
                        else
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Để Làm HotDog [Trạng Thái: ~r~Ngưng Bán~w~]')
                        end
                        if IsControlJustPressed(0, 38) then
                            StartHotdogMinigame()
                        end
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            QBCore.Functions.Notify('Bạn đang ở xa chiếc xe HotDog của bạn..', 'error')
        end
    else
        QBCore.Functions.Notify('Bạn không có xe HotDog nào', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
	    ...
	    SellingData.Hotdog = nil
            QBCore.Functions.Notify('Bạn đang ở quá xa xe HotDog của bạn ...', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 4 then
            if SellingData.Hotdog ~= nil then
                ...
                QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Bán '..HotdogsForSale..'x với giá $'..(HotdogsForSale * SellingPrice)..' / [8] Từ Chối')
                if IsControlJustPressed(0, 161) or IsDisabledControlJustPressed(0, 161) then
                    QBCore.Functions.Notify(HotdogsForSale..'x Hotdog Đã Bán Với Giá $'..(HotdogsForSale * SellingPrice), 'success')
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    QBCore.Functions.Notify('Bạn Đã Từ Chối Bán!', 'error')
                    SellingData.HasTarget = false
	    	    ...
	    	    SellingData.HasTarget = false
	    	    SellingData.Hotdog = nil
                QBCore.Functions.Notify('Bạn đang ở quá xa xe Hot Dog của bạn ...', 'error')
                break
            end
        else
            SellingData.HasTarget = false
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        if Config.MyLevel == 1 then
            QBCore.Functions.Notify('Bạn có 1 '..Config.Stock[Quality].Label..' Hot Dog được làm thành công!')
            ...
            if Luck == LuckyNumber then
                QBCore.Functions.Notify('Bạn Có '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hot Dog được làm thành công!')
        ...
        QBCore.Functions.Notify('Bạn không có ('..Config.Stock[Quality].Label..') more about this in front of council..')
    end
    PreparingFood = false
end
    ...
    if DoesEntityExist(StandObject) then
        QBCore.Functions.TriggerCallback('qb-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                QBCore.Functions.Notify('Bạn đã nhận lại $250 Tiền Cọc Xe Hot Dog!', 'success')
            else
                QBCore.Functions.Notify('Something is my friend..', 'error')
            end
        end)
    else
        QBCore.Functions.Notify('Không thấy quầy xúc xích của bạn đâu cả, Bạn sẽ không nhận lại được tiền đặt cọc!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
