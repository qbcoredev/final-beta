        ...
        while Earnings > 0 do
            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    DrawText3D(coords.x, coords.y, coords.z, "~g~E~w~ - Thanh Toán Lương")
        ...
        if isLoggedIn then
            if PlayerJob.name == "garbage" then
                if distance < 10.0 then
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Cất Xe Rác")
                            if IsControlJustReleased(0, 38) then
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        QBCore.Functions.Notify("Bạn có $500, - tiền đặt cọc được trả lại!")
                                    else
                                        QBCore.Functions.Notify("Bạn không có khoản tiền cọc nào được trả cho chiếc xe này..")
                                    end
                                end)
                            end
                        else
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Lấy Xe Rác")
                            if IsControlJustReleased(0, 38) then
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        ...
                                        QBCore.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            SetVehicleNumberPlateText(veh, "XERAC"..tostring(math.random(1000, 9999)))
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            QBCore.Functions.Notify("Đã Cọc $500 cho chiếc xe này!")
                                            QBCore.Functions.Notify("Bạn đã bắt đầu làm việc, vị trí được đánh dấu trên GPS!")
                                        end, coords, true)
                                    else
                                        QBCore.Functions.Notify("Bạn không có đủ tiền để gửi .. Chi phí gửi là $1000,-")
                                    end
                                end)
                            end
                        end
                            ...
                            if Distance < 20 or hasGarbageBag then
                                ...
                                if not hasGarbageBag then
                                    ...
                                    if DoesEntityExist(GarbageVehicle) then
                                        ...
                                        if TruckDist < 2 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "~g~E~w~ - Vứt bỏ túi rác")
                                            if IsControlJustPressed(0, 51) then
                                                hasGarbageBag = false
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    Earnings = Earnings + math.random(150, 170)
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        QBCore.Functions.Notify("Tất cả các túi rác đã được thu gom, tiến tới vị trí tiếp theo!")
                                                    else
                                                        -- Finished work
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        QBCore.Functions.Notify("Vẫn còn "..AmountOfBags.." Túi Rác!")
                                                    else
                                                        QBCore.Functions.Notify("vẫn còn "..AmountOfBags.." Túi ở đó!")
                                                    end
                                                    hasGarbageBag = false
                                                end
                                                DeliverAnim()
