    ...
    if amount > 0 then
        ...
        if location == #Config.Locations["trashcan"] then
            for i = 1, math.random(3, 5), 1 do
                ...
                Player.Functions.AddItem(item, math.random(2, 3))
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item], 'add')
                Citizen.Wait(500)
            end
        end

        TriggerClientEvent('QBCore:Notify', src, "Bạn Có $"..amount..",- đã được trả vào tài khoản ngân hàng của bạn!", "success")
    else
        TriggerClientEvent('QBCore:Notify', src, "Bạn đã không kiếm được gì..", "error")
    end
end)