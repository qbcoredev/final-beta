	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				QBCore.Functions.Draw2DText(0.5, 0.88, 'Bấm ~g~[E]~w~ Để Hái', 0.4)
			end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				if PlayerJob.name == "police" then
					...
					QBCore.Functions.Progressbar("search_register", "Đang Hái..", 3000, false, true, {
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 5 then
			...
			if not isProcessing and GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) <1 then
				QBCore.Functions.DrawText3D(Config.CircleZones.WeedProcessing.coords.x, Config.CircleZones.WeedProcessing.coords.y, Config.CircleZones.WeedProcessing.coords.z, 'Bấm ~g~[E]~w~ Để Chế Biến')
			end
