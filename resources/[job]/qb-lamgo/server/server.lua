...
RegisterServerEvent('qb-lamgo:sell')
AddEventHandler('qb-lamgo:sell', function()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local Player = QBCore.Functions.GetPlayer(src)

    if xPlayer.Functions.RemoveItem("wood_proc", 1) then
        TriggerClientEvent("QBCore:Notify", src, "Bạn Đã Bán 1 Ván Gỗ", "success", 1000)
        Player.Functions.AddMoney("cash", Config.Prices)
        Citizen.Wait(200)
    else
        TriggerClientEvent("QBCore:Notify", src, "Bạn Không Có Gỗ", "error", 1000)
    end
end)
