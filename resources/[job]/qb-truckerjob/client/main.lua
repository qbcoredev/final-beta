            ...
            if PlayerJob.name == "trucker" then
                ...
                if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 10.0 then
                    ...
                    if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 1.5 then
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Cất Xe")
                        else
                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        QBCore.Functions.Notify('Đây không phải là một chiếc xe chở hàng!', 'error')
                                    end
                                else
                                    QBCore.Functions.Notify('Bạn phải là người lái xe để làm điều này..')
                                end
                            else
                                MenuGarage()
                ...
                if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 4.5 then
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        DrawText3D(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, "~g~E~w~ - Thanh Toán")
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
				    ...
				    ClearAllBlipRoutes()
                                ...
                                QBCore.Functions.Notify("Bạn Chưa Làm Gì Cả..", "error")
                            end
                        end
                    elseif #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 2.5 then
                        DrawText3D(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, "Thanh Toán")
                    end  
                end
                    ...
                    if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 40.0 then
                        if not IsPedInAnyVehicle(PlayerPedId()) then
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) < 1.5 and not isWorking then
                                        ...
                                        if IsControlJustReleased(0, 38) then
                                            isWorking = true
                                            QBCore.Functions.Progressbar("work_carrybox", "Đang Lấy Hàng", 2000, false, true, {
                            ...
                            elseif hasBox then
                                if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 1.5 and not isWorking then
                                    ...
                                    if IsControlJustReleased(0, 38) then
                                        ...
                                        QBCore.Functions.Progressbar("work_dropbox", "Giao Hàng", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                QBCore.Functions.Notify("Bạn Đã Giao Đủ Hàng , Đi Đến Điểm Tiếp Theo")
						    ...
						    ClearAllBlipRoutes()
                                    ...
                                    DrawText3D(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, "Giao Hàng")
                                end
                            end
                        end
                    end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        QBCore.Functions.Notify("Bạn Đã Giao Hết , Quay Về Nhận Lương!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
	    ClearAllBlipRoutes()
            CurrentBlip = nil
