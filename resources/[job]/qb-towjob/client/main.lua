...
AddEventHandler('qb-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    QBCore.Functions.Notify("Phương Tiện Phải ở Bên Phải", "error")
                    return
                end
            end
                ...
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", "Nâng xe...", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                QBCore.Functions.Notify("Chở Xe Về", "success", 5000)
                            ...
                            QBCore.Functions.Notify("Đã Nâng Xe")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
            ...
            QBCore.Functions.Progressbar("untowing_vehicle", "Gỡ Xe", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                QBCore.Functions.Notify("Đã Hạ Xe")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                QBCore.Functions.Notify("Thất Bại", "error")
            end)
        ...
        QBCore.Functions.Notify("Bạn phải ở trong một chiếc xe kéo trước", "error")
    end
end)
...
function deliverVehicle(vehicle)
    ...
    QBCore.Functions.Notify("đi kéo chiếc xe khác")
