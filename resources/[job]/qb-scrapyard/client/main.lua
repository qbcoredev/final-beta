...
Citizen.CreateThread(function()
	for id, scrapyard in pairs(Config.Locations) do
        ...
        AddTextComponentSubstringPlayerName("~r~Khu Vực Rã Xe")
		...
		if closestScrapyard ~= 0 then
				...
				if IsPedInAnyVehicle(PlayerPedId()) then
					...
					if vehicle ~= 0 and vehicle ~= nil then 
						...
						if #(pos - vector3(vehpos.x, vehpos.y, vehpos.z)) < 2.5 and not isBusy then
							DrawText3Ds(vehpos.x, vehpos.y, vehpos.z, "~g~E~w~ - Phá Hủy Phương Tiện")
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										ScrapVehicle(vehicle)
									else
										QBCore.Functions.Notify("Rã Xe Thất Bại Do Đã Rã Hoặc Không Có Trong Danh Sách", "error")
									end
								else
			...
			if #(pos - vector3(Config.Locations[closestScrapyard]["list"].x, Config.Locations[closestScrapyard]["list"].y, Config.Locations[closestScrapyard]["list"].z)) < 1.5 then
				if not IsPedInAnyVehicle(PlayerPedId()) and not emailSend then
					DrawText3Ds(Config.Locations[closestScrapyard]["list"].x, Config.Locations[closestScrapyard]["list"].y, Config.Locations[closestScrapyard]["list"].z, "~g~E~w~ - Nhận Danh Sách Xe")
					if IsControlJustReleased(0, 38) then
						CreateListEmail()
					end
				end
...
function ScrapVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("scrap_vehicle", "Phá hủy phương tiện", scrapTime, false, true, {
