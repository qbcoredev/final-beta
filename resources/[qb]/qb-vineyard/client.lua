				...
				if nearlocation <= 15 then
					...
					if nearlocation <= 3 then
						if not startVineyard then
							DrawText3Ds(-1928.81, 2059.53, 140.84, "[E] Bắt đầu chọn nho") 
							
								if IsControlJustReleased(0,38) then
									if PlayerJob.name == "vineyard" then
										startVineyard = true
									else
										QBCore.Functions.Notify("Tôi không nghĩ rằng tôi làm việc ở đây...", "error")
									end
								end
						end
					end
...
function CreateBlip()
    ...
    AddTextComponentString("Vườn Nho")
...
function pickProcess()
    QBCore.Functions.Progressbar("pick_grape", "Hái nho ..", math.random(6000,8000), false, true, {
						...
						if not wineStarted then
							if not loadIngredients then
								if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
									DrawText3Ds(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y,  Config.Vineyard["wine"].coords.z + 0.2, '[E] Tải thành phần')
									if IsControlJustPressed(0, 38) then
										if PlayerJob.name == "vineyard" then
											TriggerServerEvent("qb-vineyard:server:loadIngredients")
										else
											QBCore.Functions.Notify("Tôi Không Nghĩ Nên Làm Việc Ở Đây...", "error")
										end
									end
								end
							else
								if not finishedWine then
									if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
										DrawText3Ds(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z + 0.2, '[E] Bắt Đầu Làm')
										if IsControlJustPressed(0, 38) then
											if PlayerJob.name == "vineyard" then
												StartWineProcess()
											else
									...
									if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
										DrawText3Ds(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z + 0.2, '[E] Nhận rượu vang')
										if IsControlJustPressed(0, 38) then
											if PlayerJob.name == "vineyard" then
							...
							DrawText3Ds(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z - 0.4, 'Sẵn sàng '..winetimer..'s')
						end
					end
				end
			...
			if nearlocation <= 15 then
				...
				if nearlocation <= 3 then	
					...
					if #(pos - vector3(Config.Vineyard["grapejuice"].coords.x, Config.Vineyard["grapejuice"].coords.y, Config.Vineyard["grapejuice"].coords.z)) < 1 then
						DrawText3Ds(Config.Vineyard["grapejuice"].coords.x, Config.Vineyard["grapejuice"].coords.y,  Config.Vineyard["grapejuice"].coords.z + 0.2, '[E] Làm nước nho')
						if IsControlJustPressed(0, 38) then
							if PlayerJob.name == "vineyard" then
...
function grapeJuiceProcess()
    QBCore.Functions.Progressbar("grape_juice", "Chế biến nho ..", math.random(15000,20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
