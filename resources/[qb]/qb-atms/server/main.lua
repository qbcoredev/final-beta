...
Citizen.CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('QBCore:Notify', -1, "Đặt lại giới hạn rút tiền hàng ngày", "success")
    end
end)
        ...
        if dailyWith < Config.DailyLimit then
            ...
            if xCH ~= nil then
                ...
                if bankCount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', src, "Rút $" .. data.amount .. 'từ thẻ tín dụng. Rút tiền hàng ngày: ' .. dailyWithdraws[cardHolder], "success")
                else
                    TriggerClientEvent('QBCore:Notify', src, "Bạn không thể sử dụng số âm trong ATM.", "error")
                end
...
RegisterServerEvent('qb-atms:server:uATM')
AddEventHandler('qb-atms:server:uATM', function(data)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
