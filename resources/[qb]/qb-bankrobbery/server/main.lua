        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 ...
                 elseif Config.RewardTypes[itemType].type == "money" then
                    ...
                    ply.Functions.AddItem('goldbar', math.random(2,3), false, info)
            ...
            ply.Functions.AddItem('goldbar', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], "add")
        end
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 ...
                 elseif Config.RewardTypes[itemType].type == "money" then
                    ...
                    ply.Functions.AddItem('goldbar', math.random(1,4), false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], "add")
                 end
            else
                ply.Functions.AddItem('goldbar', 1)
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], "add")
                 ...
                 ply.Functions.AddItem("goldbar", 1, false, info)
                 TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], "add")
            end
        else
...
AddEventHandler('qb-bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    ...
    local data = {displayCode = '211', description = 'Cướp Ngân Hàng', isImportant = 0, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Có Ngân Hàng Đang Bị Cướp'}
...
QBCore.Functions.CreateUseableItem("thermite", function(source, item)
	...
	if Player.Functions.GetItemByName('lighter') ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, "Bạn đang thiếu nguồn đánh lửa ", "error")
    end
end)
