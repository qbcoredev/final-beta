                            ...
                            if not Config.SmallBanks[closestBank]["lockers"][k]["isOpened"] then
                                if lockerDist < 5 then
                                    ...
                                    if lockerDist < 0.5 then
                                        DrawText3Ds(Config.SmallBanks[closestBank]["lockers"][k]["coords"].x, Config.SmallBanks[closestBank]["lockers"][k]["coords"].y, Config.SmallBanks[closestBank]["lockers"][k]["coords"].z + 0.3, '[E] Tiến Hành Phá Tủ')
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumFleecaPolice then
                                                openLocker(closestBank, k)
                                            else
                                                QBCore.Functions.Notify('Cần Tối Thiểu '..Config.MinimumFleecaPolice..'Cảnh Sát', "error") 
                                            end
                                        end
                                    end
                                end
    ...
    if closestBank ~= nil then
        QBCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                        ...
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            if not Config.SmallBanks[closestBank]["isOpened"] then 
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                    if result then 
                                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                        QBCore.Functions.Progressbar("hack_gate", "Kết Nối Thiết Bị Hacking ..", math.random(5000, 10000), false, true, {
                                        ...
                                        QBCore.Functions.Notify("Đang Thiếu Vật Phẩm Nào Đó ..", "error")
                                    end
                                end, "trojan_usb")
                            else
                                QBCore.Functions.Notify("Ngân Hàng Đã Bị Đánh Cắp Trước Đó ..", "error")
                            end
                        else
                            QBCore.Functions.Notify('Cần Tối Thiểu  '..Config.MinimumFleecaPolice..' Cảnh Sát', "error")
                        end
                ...
                QBCore.Functions.Notify("Hiện Tại Đã Có Ngân Hàng Bị Đột Nhập, Khóa Bảo Mật Toàn Hệ Thống Được Kích Hoạt.", "error", 5500)
            end
        end)
    end
end)
    ...
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                QBCore.Functions.Progressbar("open_locker_drill", "Đang Phá Khóa Tủ ..", math.random(18000, 18000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                ...
                QBCore.Functions.Notify("Có Vẻ Khóa Bảo Mật Quá Tốt ..", "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        ...
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", "Đang Phá Khóa Tủ ..", math.random(18000, 18000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
AddEventHandler('qb-bankrobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then 
        ...
        AddTextComponentString("~r~Cảnh Báo An Ninh: Cướp Bank")
