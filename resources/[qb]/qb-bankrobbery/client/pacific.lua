                        ...
                        if not Config.BigBanks["pacific"]["lockers"][k]["isOpened"] then
                            if lockerDist < 5 then
                                ...
                                if lockerDist < 0.5 then
                                    DrawText3Ds(Config.BigBanks["pacific"]["lockers"][k]["coords"].x, Config.BigBanks["pacific"]["lockers"][k]["coords"].y, Config.BigBanks["pacific"]["lockers"][k]["coords"].z + 0.3, '[E] Tiến Hành Phá Tủ')
                                    if IsControlJustPressed(0, 38) then
                                        if CurrentCops >= Config.MinimumPacificPolice then
                                            openLocker("pacific", k)
                                        else
                                            QBCore.Functions.Notify('Cần Tối Thiểu '..Config.MinimumPacificPolice..' Cảnh Sát', "error")
                                        end
                                    end
                                end
                            end
                        ...
                        if not Config.BigBanks["pacific"]["isOpened"] then 
                            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                if result then 
                                    ...
                                    QBCore.Functions.Notify("Bạn đang thiếu vật phẩm ..", "error")
                                end
                            end, "trojan_usb")
                        else
                            QBCore.Functions.Notify("Ngân Hàng Đã Bị Đột Nhập", "error")
                        end
                    else
                        QBCore.Functions.Notify('Cần Tối Thiểu '..Config.MinimumPacificPolice..' Cảnh Sát', "error")
                    end
        ...
        QBCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        QBCore.Functions.Progressbar("security_pass", "Vui Lòng Xác Nhận..", math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
