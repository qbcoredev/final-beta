                        ...
                        if dist < 30 then
                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    DrawText3Ds(Config.Locations[case]["coords"]["x"], Config.Locations[case]["coords"]["y"], Config.Locations[case]["coords"]["z"], '[E] Cướp Trang Sức')
                                        ...
                                        QBCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    ...
                                                    QBCore.Functions.Notify('Vũ khí này không thể cướp..', 'error')
                                                end
                                            else
                                                QBCore.Functions.Notify('Cảnh sát không đủ , Cần('.. Config.RequiredCops ..') ', 'error')
                                            end                
                                        end)
                                    end
                                end
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        QBCore.Functions.Notify("Bạn đã để lại một dấu vân tay trên kính", "error")
    end
...
AddEventHandler('qb-jewellery:client:PoliceAlertMessage', function(title, coords, blip)
    if blip then
        ...
        AddTextComponentString("911 - Tình huống đáng ngờ tại cửa hàng trang sức")
