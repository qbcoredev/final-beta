    ...
    while true do
        ...
        if dist < 20 then
            ...
            if #(pos - vector3(Config.Cityhall.coords.x, Config.Cityhall.coords.y, Config.Cityhall.coords.z)) < 1.5 then
                DrawText3Ds(Config.Cityhall.coords, '~g~E~w~ -Truy Cập')
...
RegisterNUICallback('requestId', function(data)
    if inRange then
        ...
        QBCore.Functions.Notify('Bạn đã nhận được '..idTypes[idType].label..' phí $50', 'success', 3500)
    else
...
RegisterNUICallback('applyJob', function(data)
    if inRange then
        ...
        QBCore.Functions.Notify('Thật không may sẽ không hoạt động ...', 'error')
    end
end)
