...
AddEventHandler('qb-cityhall:server:sendDriverTest', function()
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Một email đã được gửi đến các trường dạy lái xe và bạn sẽ được liên hệ tự động', "success", 5000)
end)
...
AddEventHandler('qb-cityhall:server:ApplyJob', function(job)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Chúc mừng công việc mới của bạn! ('..JobInfo.label..')')
end)
...
AddEventHandler('qb-cityhall:server:banPlayer', function()
    ...
    DropPlayer(src, 'Cố gắng khai thác.')
end)
