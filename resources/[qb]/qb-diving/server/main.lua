local CoralTypes = {
    ["dendrogyra_coral"] = math.random(150, 150),
    ["antipatharia_coral"] = math.random(100, 100),
}
...
AddEventHandler('qb-diving:server:BuyBoat', function(boatModel, BerthId)
    ...
    elseif PlayerMoney.bank >= BoatPrice then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Không đủ tiền, bạn đang thiếu $'..missingMoney..'', 'error')
    end
end)
...
QBCore.Commands.Add("divingsuit", "Cởi bỏ bộ đồ lặn của bạn", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
AddEventHandler('qb-diving:server:SellCoral', function()
    ...
    if HasCoral(src) then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Bạn không có san hô để bán..', 'error')
    end
end)
