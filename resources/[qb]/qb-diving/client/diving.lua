                            ...
                            if not CoralLocation.PickedUp then
                                ...
                                if CoralDistance <= 1.5 then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        ...
                                        QBCore.Functions.Progressbar("take_coral", "Thu thập san hô", times * 1000, false, true, {
...
AddEventHandler('qb-diving:client:UseGear', function(bool)
    if bool then
        GearAnim()
        QBCore.Functions.Progressbar("equip_gear", "Mặc một bộ đồ lặn", 5000, false, true, {}, {}, {}, {}, function() -- Done
            DeleteGear()
            ...
            TriggerEvent('chatMessage', "SYSTEM", "error", "/divingsuit để cởi bộ đồ lặn")
        end)
    else
        if currentGear.enabled then
            GearAnim()
            QBCore.Functions.Progressbar("remove_gear", "Cởi bộ đồ lặn ..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                DeleteGear()
                ...
                ClearPedTasks(PlayerPedId())
                QBCore.Functions.Notify('Bạn đã cởi bộ đồ lặn của mình')
            end)
        else
            QBCore.Functions.Notify('Bạn không mặc một thiết bị lặn ..', 'error')
        end
    end
end)
