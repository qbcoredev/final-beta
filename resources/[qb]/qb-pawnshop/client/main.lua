...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName("Chợ Đen")
			...
			if #(pos - Config.PawnLocation) < 1.5 then
				if GetClockHours() >= 7 and GetClockHours() <= 17 then
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						DrawText3D(Config.PawnLocation.x, Config.PawnLocation.y, Config.PawnLocation.z, "~g~E~w~ - Bán đồng hồ / dây chuyền / nhẫn ($"..sellPrice..")")
						...
						DrawText3D(Config.PawnLocation.x, Config.PawnLocation.y, Config.PawnLocation.z, "Chợ Đen: Bạn không có gì để bán")
					end
				else
					DrawText3D(Config.PawnLocation.x, Config.PawnLocation.y, Config.PawnLocation.z, "Chợ Đen đã đóng cửa, mở ra từ ~r~7:00")
				end
			end
		end
		if not inRange then
			...
			if #(pos - Config.PawnHardwareLocation) < 1.5 then
				if GetClockHours() >= 9 and GetClockHours() <= 16 then
					...
					elseif sellHardwareItemsSet and sellHardwarePrice ~= 0 then
						DrawText3D(Config.PawnHardwareLocation.x, Config.PawnHardwareLocation.y, Config.PawnHardwareLocation.z, "~g~E~w~ - Bán ($"..sellHardwarePrice..")")
						if IsControlJustReleased(0, 38) then
							TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_STAND_IMPATIENT", 0, true)
						...
						DrawText3D(Config.PawnHardwareLocation.x, Config.PawnHardwareLocation.y, Config.PawnHardwareLocation.z, "Chợ Đen: Bạn không có gì để bán")
					end
				else
					DrawText3D(Config.PawnHardwareLocation.x, Config.PawnHardwareLocation.y, Config.PawnHardwareLocation.z, "Chợ Đen đóng cửa, mở từ ~r~9:00")
				end
			end
		end
		if not inRange then
