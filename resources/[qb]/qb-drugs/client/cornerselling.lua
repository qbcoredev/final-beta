        ...
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if result ~= nil then
                ...
                if not cornerselling then
                    ...
                    QBCore.Functions.Notify('Đã Bắt Đầu , Chờ Khách Nào..')
                    startLocation = GetEntityCoords(PlayerPedId())
                    -- TaskStartScenarioInPlace(PlayerPedId(), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
                else
                    cornerselling = false
                    LocalPlayer.state:set("inv_busy", false, true)
                    QBCore.Functions.Notify('Bạn Đã Dừng Bán Hàng')
                    -- ClearPedTasks(PlayerPedId())
                    callPolice()
                end
            else
                QBCore.Functions.Notify('Bạn Không Có Đủ Hàng Để Bán..', 'error')
                LocalPlayer.state:set("inv_busy", false, true)
            end
        else
            QBCore.Functions.Notify("Bạn Đang Tính Làm Gì Thế ? Cần Có (".. Config.MinimumDrugSalePolice .." )Cảnh Sát Để Làm Việc Này Nhé", "error")
        end
    end)
end)
...
function toFarAway()
    QBCore.Functions.Notify('Bạn Đã Đi Quá Xa!', 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    busySelling = false
...
function callPolice()
 --   local title = policeMessage[math.random(1, #policeMessage)]
    ...
    TriggerServerEvent("qb-drugs:server:callCops", "safe", safe, streetLabel, pCoordss)
        ...
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                if pedDist < 1.5 and cornerselling then
                    QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' với $'..randomPrice..'? / ~g~G~w~ Từ Chối Đề Nghị')
                    ...
                    if IsControlJustPressed(0, 47) then
                        QBCore.Functions.Notify('Giao Dịch Bị Hủy!', 'error')
                        hasTarget = false
