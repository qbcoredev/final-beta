	...
	if QBCore.Commands.List[command] ~= nil then
		...
		if Player ~= nil then
			if (QBCore.Functions.HasPermission(source, "god")) or (QBCore.Functions.HasPermission(source, QBCore.Commands.List[command].permission)) or (QBCore.Commands.List[command].permission == Player.PlayerData.job.name) then
				...
				TriggerClientEvent('QBCore:Notify', source, "không có quyền dùng lệnh này", "error")
			end
		end
	end
end)
...
AddEventHandler('QBCore:ToggleDuty', function()
	...
	if Player.PlayerData.job.onduty then
		...
		TriggerClientEvent('QBCore:Notify', src, "Bạn Đã Dừng Ca Trực!")
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('QBCore:Notify', src, "Bạn Đã Bắt Đầu Ca Trực!")
	end
...
QBCore.Functions.CreateCallback('police:online', function(source, cb)
