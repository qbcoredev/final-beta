                        ...
                        if dist <= 1.5 then
                            ...
                            if CurrentCops >= Config.MinimumHouseRobberyPolice then
                                if Config.Houses[k]["opened"] then
                                    DrawText3Ds(Config.Houses[k]["coords"]["x"], Config.Houses[k]["coords"]["y"], Config.Houses[k]["coords"]["z"], '~g~E~w~ - Truy Cập')
                ...
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        if PlayerJob.name == "police" then
                            QBCore.Functions.Notify('Cảnh Sát không Thể Làm Việc Này', 'error')
                        else
                            PoliceCall()
                        ...
                        QBCore.Functions.Notify('Cánh cửa đã mở..', 'error', 3500)
                        print("1")
                    end
                else
                    QBCore.Functions.Notify('Không Có Đủ Cảnh Sát Làm Việc..', 'error', 3500)
                end
            end
        else
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        if CurrentCops >= Config.MinimumHouseRobberyPolice then
                            if cooldown then
                                QBCore.Functions.Notify('Vui Lòng Chờ 5 Phút..', 'error', 3500)
                            else
                                if not Config.Houses[closestHouse]["opened"] then
                                    if PlayerJob.name == "police" then
                                        QBCore.Functions.Notify('Cảnh Sát không Thể Làm Việc Này', 'error')
                        ...
                        QBCore.Functions.Notify('Có vẻ như bạn đang thiếu một cái gì đó...', 'error', 3500)
                    end
                end
            end, "lockpick")
        end
...
function PoliceCall()
    ...
    if closestPed ~= nil then
        ...
        TriggerServerEvent("qb-houserobbery:server:callCops", "safe", streetLabel, pos)
    end
end
...
function lockpickFinish(success)
    if success then
        ...
        TriggerServerEvent("qb-houserobbery:server:callCops", "safe", safe, streetLabel, pos)
        ...
        QBCore.Functions.Notify('Nó không hoạt động..', 'error', 2500)
    end
end
