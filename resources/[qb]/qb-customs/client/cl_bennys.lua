                ...
                if nearDefault then
                    ...
                    if nearDefault then
                        if not isPlyInBennys then
                            Draw3DText(v.coords.x, v.coords.y, v.coords.z + 0.5, "[Nhấn ~g~E~w~ - Truy Cập]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                            if IsControlJustReleased(1, 38) then
                                if PlayerJob.name == "doxe" then
                                    TriggerEvent('event:control:bennys', k)
                                else
                                    QBCore.Functions.Notify("Bạn không được ủy quyền", "error")
                                end
                            end
                        else
                            disableControls()
