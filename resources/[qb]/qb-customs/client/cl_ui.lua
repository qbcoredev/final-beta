...
function InitiateMenus(isMotorcycle, vehicleHealth)
    ...
    --#[Repair Menu]#--
    if vehicleHealth < 900.0 then
        local repairCost = math.ceil(900 - vehicleHealth)

        TriggerServerEvent("qb-customs:updateRepairCost", repairCost)
        createMenu("repairMenu", "Chào Mừng Bạn Đến Độ Xe Benny's", "XE BỊ HỎNG QUÁ 90% SẼ KHÔNG THỂ ĐỘ")
        --populateMenu("repairMenu", -1, "Repair", "$" .. repairCost)
        finishPopulatingMenu("repairMenu")
    end

    --#[Main Menu]#--
    createMenu("mainMenu", "Chào Mừng Bạn Đến Độ Xe Benny's", "Muốn Độ Gì Nào ?")
