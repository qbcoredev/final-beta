...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        ...
        if Config.Stores[k].shopType == "barber" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Tiệm Tóc")
        ...
        if Config.Stores[k].shopType == "surgeon" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Phẫu Thuật Thẩm Mỹ")
		...
		local dist = #(pos - Config.Stores[k].coords)
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                 DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ -Truy Cập Shop Quần Áo')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Hớt Tóc')
                            elseif Config.Stores[k].shopType == "surgeon" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Phẫu Thuật')
                            end
                            if IsControlJustPressed(0, 38) then -- E
                                ...
                                elseif Config.Stores[k].shopType == "surgeon" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Đặc Tính", selected = true},
                                    })
                                end
                            end
                        end
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].coords.x, Config.ClothingRooms[k].coords.y, Config.ClothingRooms[k].coords.z + 0.3, '~g~E~w~ - Xem Quần Áo')
                                if IsControlJustPressed(0, 38) then -- E
                                ...
                                if PlayerData.gang.name == Config.ClothingRooms[k].requiredJob then
                                    ...
                                    if IsControlJustPressed(0, 38) then -- E
                                        ...
                                        QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                            openMenu({
                                                {menu = "roomOutfits", label = "Cài Đặt Trước", selected = true, outfits = Config.Outfits[PlayerData.gang.name][gender]},
                                                {menu = "myOutfits", label = "Trang Phục Của Tôi", selected = false, outfits = result},
                                                {menu = "character", label = "Quần Áo", selected = false},
                                                {menu = "accessoires", label = "Phụ kiện", selected = false}
                                            })
...
AddEventHandler('qb-clothing:client:openMenu', function()
    ...
    openMenu({
        ...
        {menu = "clothing", label = "Đặc tính", selected = false},
        {menu = "accessoires", label = "Phụ kiện", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    QBCore.Functions.Notify("Bạn không thể tháo vòng đeo tay của mình ..", "error")
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    QBCore.Functions.Notify("Bạn đã xóa"..data.outfitName.." outfit!")
end)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        QBCore.Functions.Notify("Bạn đã lựa chọn "..oData.outfitName.."! Nhấn để xác nhận trang phục.")
    end
end)
