                ...
                for k, v in pairs(Gates) do
                    ...
                    if (dist < 1.5) then
                        ...
                        if securityLockdown then
                            DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "~r~Hệ thống khóa ")
                        elseif Gates[k].hit then
                            DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "Vi phạm hệ thống.")
...
Citizen.CreateThread(function()
	while true do
        ...
        if #(pos - vector3(Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y, Config.Locations["middle"].coords.z)) > 200 and inJail then
            ...
            QBCore.Functions.Notify("Bạn đã trốn thoát ... ra khỏi đây.!", "error")
		end
	end
end)
...
AddEventHandler('electronickit:UseElectronickit', function()
    if currentGate ~= 0 and not securityLockdown and not Gates[currentGate].hit then 
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
            if result then 
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                QBCore.Functions.Progressbar("hack_gate", "Đang Sửa Điện..", math.random(5000, 10000), false, true, {
                ...
                QBCore.Functions.Notify("Bạn Không Có Bộ Sửa Điện..", "error")
            end
        end, "gatecrack")
    end
end)
...
AddEventHandler('prison:client:SetLockDown', function(isLockdown)
    ...
    if securityLockDown and inJail then
        TriggerEvent("chatMessage", "HOSTAGE", "error", "Báo Động Đã Bật!")
    end
end)
