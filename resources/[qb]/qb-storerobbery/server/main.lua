local SafeCodes = {}
local cashA = 1000 				--<<how much minimum you can get from a robbery
...
AddEventHandler('qb-storerobbery:server:takeMoney', function(register, isDone)
	...
	Player.Functions.AddItem('goldbar', bags, false, info)
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], "add")
        if math.random(1, 100) <= 10 then
            ...
            if Config.Safes[Config.Registers[register].safeKey].type == "keypad" then
                info = {
                    label = "Mã Két: "..tostring(code)
                }
            else
                info = {
                    label = "Mã Két: "..tostring(math.floor((code[1] % 360) / 3.60)).."-"..tostring(math.floor((code[2] % 360) / 3.60)).."-"..tostring(math.floor((code[3] % 360) / 3.60)).."-"..tostring(math.floor((code[4] % 360) / 3.60)).."-"..tostring(math.floor((code[5] % 360) / 3.60))
                }
            end
...
AddEventHandler('qb-storerobbery:server:SafeReward', function(safe)
    ...
    if luck <= 10 then
        Player.Functions.AddItem("tosti", math.random(2, 4))
...
--[[     local alertData = {
    ...
    local data = {displayCode = '211', description = 'Cướp Tạp Hóa', isImportant = 0, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Có Một Tạp Hóa Đang Bị Cướp (CAMERA ID: '..cameraId..')'}
