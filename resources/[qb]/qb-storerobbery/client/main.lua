    ...
    while true do
        ...
        for k, v in pairs(Config.Registers) do
            ...
            if dist <= 1 and Config.Registers[k].robbed then
                inRange = true
                DrawText3Ds(Config.Registers[k][1].xyz, 'Máy tính tiền trống rỗng')
            end
        end
        if not inRange then
            Citizen.Wait(2000)
                ...
                if dist < 3 then
                    ...
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            DrawText3Ds(Config.Safes[safe][1].xyz, '~g~E~w~ - Mở Khóa')
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    QBCore.Functions.Notify("Cần Có Cảnh Sát Để Thực Hiện (Yêu Cầu ".. Config.MinimumStoreRobberyPolice ..")", "error")
                                end
                            end
                        else
                            DrawText3Ds(Config.Safes[safe][1].xyz, 'Két Sắt Đã Mở')
                        end
                    end
                end
            end
...
function takeAnim()
    ...
    TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
        timeOut = 5000,
        alertTitle = "10-31 | Cửa Hàng Bị Cướp",
        coords = {
            x = coords.x,
            y = coords.y,
            z = coords.z,
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        ...
        QBCore.Functions.Progressbar("search_register", "đang cướp..", lockpickTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
          --  TriggerClientEvent('QBCore:Notify', source, "đang có một vụ cướp diễn ra")
        }, {
            ...
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            QBCore.Functions.Notify("Đã Hủy Hành Động..", "error")
            currentRegister = 0
        end)
        Citizen.CreateThread(function()
...
RegisterNUICallback('fail', function()
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        QBCore.Functions.Notify("Bạn Đã Làm Hỏng Lockpick")
    end
    lockpick(false)
end)
...
AddEventHandler('qb-storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" then
        ...
        AddTextComponentString("10-31 | Cửa Hàng Bị Cướp")
