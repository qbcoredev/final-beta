...
RegisterCommand('radialmenu', function()
    if QBCore.Functions.GetPlayerData().metadata['inlaststand'] or QBCore.Functions.GetPlayerData().metadata['isdead']then
        QBCore.Functions.Notify("Không Thể Thực Hiện Khi Đang Bất Tỉnh", "error")
    else
        openRadial(true)
...
AddEventHandler('qb-radialmenu:client:latxe:vehicle', function()
    ...
    --if IsPedInAnyVehicle(ped, false) then vehicle = GetVehiclePedIsIn(ped, false) else vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 5.0, 0, 71) end
        ...
        QBCore.Functions.Notify('Không Thể Luật Xe', 'error', 2500) 
    end
end)

RegisterNetEvent('qb-radialmenu:client:givekey')
AddEventHandler('qb-radialmenu:client:givekey', function()
    local src = source
    local closestPlayer = GetClosestPlayer()
	target = GetPlayerServerId(closestPlayer)
    TriggerEvent('vehiclekeys:client:GiveKeys', src, target)
end)