        ...
        if isLoggedIn and ClosestHouse ~= nil then
            if InApartment then
                ...
                --[[ if logoutdist < 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) then -- E
                        if cooldown then
                            QBCore.Functions.Notify('Bạn Không Thể Đăng Xuất Bây Giờ', 'error')
                        else
                ...
                if doorbelldist < 1.2 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.doorbell.x, Apartments.Locations[ClosestHouse].coords.doorbell.y, Apartments.Locations[ClosestHouse].coords.doorbell.z, '~g~G~w~ - Bấm Chuông')
                    if IsControlJustPressed(0, 47) then -- G
                        MenuOwners()
                ...
                if IsOwned then
                   if entrance < 1.2 then
                        QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y, Apartments.Locations[ClosestHouse].coords.enter.z, '~g~E~w~ - Vào Nhà')
                        if IsControlJustPressed(0, 38) then -- E
                ...
                elseif not IsOwned then
                    if entrance < 1.2 then
                        QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y, Apartments.Locations[ClosestHouse].coords.enter.z, '~g~E~w~ - Đổi Căn Hộ ( Phí 50K$ )')
                        if IsControlJustPressed(0, 38) then -- E
...
AddEventHandler('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if rangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            QBCore.Functions.Notify("bạn đứng quá xa chuông cửa")
            return
        end
    end
...
AddEventHandler('apartments:client:RingDoor', function(player, house)
    ...
    QBCore.Functions.Notify("Có ai đó ngoài cửa!")
end)
...
function MenuOwners()
    ...
    MenuTitle = "Owners"
    ClearMenu()
    Menu.addButton("Bấm chuông", "OwnerList", nil)
...
function MenuOutfits()
    ...
    ClearMenu()
    Menu.addButton("Trang Phục", "OutfitsLijst", nil)
    Menu.addButton("Đóng Menu", "closeMenuFull", nil) 
end
...
function OutfitsLijst()
    QBCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
            QBCore.Functions.Notify("Bạn không lưu Trạng phục nào...", "error", 3500)
            closeMenuFull()
        else
...
function selectOutfit(oData)
    ...
    QBCore.Functions.Notify(oData.outfitname.." Chọn", "success", 2500)
    closeMenuFull()
    changeOutfit()
end
...
function removeOutfit(oData)
    ...
    QBCore.Functions.Notify(oData.outfitname.." Xóa", "success", 2500)
    closeMenuFull()
end
